...
class Transfromer_Baseline(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.args.bool_crf:
        #     if self.training:
        #         x = -1 * self.crf(x, x['labels'], mask=x['masks'].byte())# loss function
        #     else:
        #         x = self.crf.decode(x, x['masks'].byte())
...
class Transfromer_Baseline_2class(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        self.pretrain_model_name = args.base_model.lower()
        self.tokenizer = pretrain_token_dict[self.pretrain_model_name].from_pretrained(args.model_path)
        base_model = pretrain_model_dict[self.pretrain_model_name].from_pretrained(args.model_path)
        self.base_model_embeddings = base_model.embeddings
        self.base_model_encoder = base_model.encoder
        # self.base_model_pooler = base_model.pooler
        # BertTokenizer.from_pretrained('bert-base-uncased')
        ...
        if not args.head == '':
            self.heads = heads_dict[args.head](self.configuration.to_dict()['hidden_size'], args.head_out_channels)
            self.classifier = nn.Linear(
                self.heads.out_channels, args.num_classes)
        else:
        ...
        self.entity_feat_module = entity_feat_module_dict[self.entity_feat_module_name](self.configuration.to_dict()['hidden_size'])
        feats_channels_num = self.configuration.to_dict()['hidden_size'] * self.entity_feat_module.feats_num_coef

        self.feat_process_module = feat_process_module_dict[args.feat_process_module](feats_channels_num, args.num_fc)
    ...
    def find_entity_predicate_i(self, prob, predicate_i):
        ...
        for subject_start_i in subject_start:
            subject_end_i = subject_end[np.argwhere(subject_end >= subject_start_i)[:, 0]]
        ...
        for object_start_i in object_start:
            object_end_i = object_end[np.argwhere(object_end >= object_start_i)[:, 0]]
        ...
        for subject_i in subject_list:
            for object_i in object_list:
                spo_list.append([subject_i[0], subject_i[1], object_i[0], object_i[1], predicate_i_label])
    ...
    def find_entity_all(self, prob, seq_len):
        ...
        for batch_i in range(prob.shape[0]):
            ...
            for predicate_i in range(num_predicate_class):
                prob_predicate_i = prob_i[:, [predicate_i, predicate_i+num_predicate_class, predicate_i+num_predicate_class*2, predicate_i+num_predicate_class*3]]
                spo_predicate_i = self.find_entity_predicate_i(prob_predicate_i, predicate_i)
    ...
    def spo_feats(self, feats, spo_list):
        ...
        for batch_i in range(batch):
            ...
            for spo_index_batch_i in range(min(len(spo_list_batch_i), self.max_predicate_num)):
                ...
                feats_subject_i = feats[batch_i: batch_i+1, subject_start_index: subject_end_index + 1, :].permute(0, 2, 1)
                feats_object_i = feats[batch_i: batch_i+1, object_start_index: object_end_index + 1, :].permute(0, 2, 1)

                feats_spo_i = self.entity_feat_module(feats_subject_i, feats_object_i, feats[batch_i: batch_i+1])

                feats_spo.append(feats_spo_i)

            feats_spo_num.append(min(len(spo_list_batch_i), self.max_predicate_num))

        if len(feats_spo) > 1:
            feats_spo = torch.cat(feats_spo, dim=0)
            return feats_spo, feats_spo_num
        elif len(feats_spo) == 1 and (not self.training):
            feats_spo = torch.cat(feats_spo, dim=0)
            return feats_spo, feats_spo_num
        else:
            return torch.zeros(1, 768), [0] * batch
    ...
    def spo_labels(self, gt_spo_list, gt_predicate_num, pre_spo_list):
        ...
        for batch_i in range(batch):
            ...
            for pre_spo_index_batch_i in range(min(len(pre_spo_list_batch_i), self.max_predicate_num)):
                ...
                pre_gt_spo_dict[str(pre_spo_index_batch_i)]['pre_subject_index'] = pre_spo_i_batch_i[0:2]
                pre_gt_spo_dict[str(pre_spo_index_batch_i)]['pre_object_index'] = pre_spo_i_batch_i[2:4]
                pre_gt_spo_dict[str(pre_spo_index_batch_i)]['pre_predicate'] = pre_spo_i_batch_i[4]
                ...
                for gt_spo_index_batch_i in range(gt_predicate_num[batch_i]):
                    ...
                    if pre_spo_i_batch_i == list(gt_spo_i_batch_i):
                        ...
                        pre_gt_spo_dict[str(pre_spo_index_batch_i)]['gt_predicate'] = gt_predicate_index
                        break
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            prob_x_0_1 = torch.where(prob_x > 0.5, torch.tensor(1), torch.tensor(0)).numpy()
            pre_spo_list = self.find_entity_all(prob_x_0_1, seq_lens)

        feats_spo, pre_spo_num = self.spo_feats(x_encoder.last_hidden_state, pre_spo_list)
        ...
        if self.training:
            labels_spo = self.spo_labels(gt_predicate_labels, gt_predicate_num, pre_spo_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", default='Transfromer_Baseline_2class', type=str, help="model.")
    parser.add_argument("--base_model", default='roberta', type=str, help="Pretrain model.")
    parser.add_argument("--entity_feat_module_name", default='entity_avg_max_globalavgmax_feats', type=str, help="Pretrain model.")
    parser.add_argument("--feat_process_module", default='fc_bn_relu', type=str, help="Pretrain model.")
    parser.add_argument("--num_fc", default=2, type=int, help="Pretrain model.")
    ...
    parser.add_argument("--head_out_channels", type=int, nargs='+', default=256, help="classification classes")
    parser.add_argument("--bool_crf", action='store_true', default=False, help="whether lower_case")
    args = parser.parse_args()

    os.environ['CUDA_VISIBLE_DEVICES'] = '0'

    model = Transfromer_Baseline_2class(args)
    model_state_dict = torch.load('../log/0421/Transfromer_bert/star_end_bertbase/model_best/model_best_epoch_44_f1_88.28.pth')
    model.load_state_dict(model_state_dict, strict=False)

    tokenizer = model.tokenizer
    model = nn.DataParallel(model).cuda()
    inputs = tokenizer("In Queens , North Shore Towers , near the Nassau border , supplanted a golf course , and housing replaced a gravel quarry in Douglaston .", return_tensors="pt")
    inputs['seq_lens'] = [inputs['input_ids'].shape[1] - 2] * 2
    inputs['input_ids'] = (torch.cat([inputs['input_ids'], torch.zeros([1,128]).long()], dim=1)[:, 0: 128]).repeat(2, 1).cuda()
